...
For scenarios where the dataset may not fit in memory, we implement a \emph{vectorized loader} inspired by modern column stores such as MonetDB and Vertica. The CSV is read in fixed-size vectors (batches of 4096 rows). For each vector:

\begin{enumerate}[nosep]
    \item Parse and cast the raw CSV values into per-column arrays.
    \item Apply filter predicates column-at-a-time (predicate pushdown).
    \item If no rows survive the first predicate, skip the entire vector (short-circuit).
    \item Only surviving rows are appended to the output \texttt{ColumnStore}.
\end{enumerate}

This ensures memory usage is bounded: only one vector plus the accumulated result rows are held in memory at any time. In our run on 259,237 rows (64 vectors), 57 vectors (89.1\%) were skipped via short-circuiting, keeping only 6,532 candidates in memory (2.5\% of total rows). In the latest fully optimized runs, vectorized query peak memory was \textbf{0.40 MiB} vs.\ \textbf{1.28 MiB} for the full-load path---a 3.2$\times$ reduction.
...
Our vectorized mode loads the CSV in vectors of \textbf{4,096 rows}. For each vector, we run the same post-load transform, layout optimizations, and optimized query as the full-load path, then merge the per-vector minima for each $(x,y)$ pair. Table~\ref{tab:vec} compares this vectorized mode against the full-load optimized query.
...
\begin{itemize}[nosep]
    ...
    \item Mode~3 (vectorized): \textbf{568} valid $(x, y)$ pairs, total time $3.107 \pm 0.060$\,s, peak 0.40\,MiB
\end{itemize}
...
\paragraph{Validation Example 1:} For $(x, y) = (1, 80)$, the equivalent Excel formula is:
\begin{verbatim}
=MIN(IF((YEAR(A:A)=2020)*(MONTH(A:A)>=6)*
    (MONTH(A:A)<=6)*(town_match)*(H:H>=80),
    J:J/H:H))
\end{verbatim}

% TODO: Replace with actual screenshot of Excel validation
\begin{figure}[H]
\centering
\fbox{\parbox{0.9\textwidth}{\centering\vspace{2cm}\texttt{TODO: Insert screenshot of Excel validation for selected (x,y) pairs}\vspace{2cm}}}
\caption{Excel validation comparing program output with Excel-computed results for selected $(x, y)$ pairs.}
\label{fig:excel}
\end{figure}
...
\midrule
% TODO: Fill in actual validation data from Excel
(1, 80)  & --- & --- & --- \\
(3, 100) & --- & --- & --- \\
(5, 120) & --- & --- & --- \\
(7, 85)  & --- & --- & --- \\
...
Name and Signature of Member 1 & Name and Signature of Member 2 \\[2cm]
\rule{6cm}{0.4pt} & \rule{6cm}{0.4pt} \\
